...
class RunTracker:
    ...
    def finalize_metrics(self) -> None:
        """Compute derived fields on ``self.metrics``.

        Must run before persisting metrics to the DB. ``__exit__`` also calls
        this so MLflow gets the same finalized snapshot.
        """
        m = self.metrics
        if self._t0:
            m.latency_ms = (time.time() - self._t0) * 1000.0
